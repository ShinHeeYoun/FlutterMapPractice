...
- **2026-06-19 16:50:** 목적지 도착 알림 내역/통계 기능 및 실시간 이동 경로 추적, 직관적인 알림 제어 패널(ActiveAlarmPanel) 도입.
  - **알림 내역 및 통계:** `sqflite`를 도입하여 사용자가 목적지 알림을 사용할 때마다 자동으로 이동 경로(좌표 배열), 총 소요시간, 이동거리, 평균 속도 등을 로컬 DB에 기록. 메뉴의 기록 보기 탭을 통해 상세 내역과 맵에 그려진 과거 궤적 확인 기능 지원.
  - **실시간 이동 경로 표시:** 목적지로 향하는 직선(목표 경로)과 구별되도록, 실제 사용자가 이동한 궤적을 실시간 위치 스트림을 바탕으로 빨간색 Polyline으로 맵에 렌더링.
  - **슬라이딩 제어 패널:** 알림 활성화 시 하단에 목적지 이름, 남은 거리, 알림 끄기 버튼이 포함된 `ActiveAlarmPanel` 표출. 스와이프 제스처를 통해 열고 닫을 수 있으며(Peek/Expand 상태 지원), 패널 높이에 맞춰 지도의 패딩(Dynamic Padding)이 자동 조절되어 직선 경로가 가려지지 않게 UX 개선.
  - **인라인 반경 설정:** 기존 바텀 시트 형태의 알림 반경 설정창 대신 `ActiveAlarmPanel` 내부에 슬라이더를 직접 배치. 실시간으로 반경을 조절하고 즉각적으로 백그라운드 위치 추적 조건에 반영 가능.
